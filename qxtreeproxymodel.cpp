...
/*!
  ...
  \li needs to have an id column and a parent column (and optional more columns)
  \li id column needs to be unique, in sqlite PRIMARY KEY
  \li id column and parent column need to be integers (or strings convertible to integer)
  \li id column must not be 0 (nor "0" or anything else that would be converted to zero)
  \li parent column may be empty (which is equivalent to zero) or refer to a valid parent, i.e., have a number that is the id of another record
  \li records must not be circularly connected through their id and parent columns
