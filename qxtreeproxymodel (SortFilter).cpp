...
/*
class QXTreeProxyModelPrivate : public QAbstractProxyModelPrivate
{
    Q_DECLARE_PUBLIC(QXTreeProxyModel)
...
typedef QHash<QModelIndex, QXTreeProxyModelPrivate::Mapping *> IndexMap;

void QXTreeProxyModelPrivate::_q_sourceModelDestroyed()
{
    QAbstractProxyModelPrivate::_q_sourceModelDestroyed();
    clear_mapping();
}

void QXTreeProxyModelPrivate::remove_from_mapping(const QModelIndex &source_parent)
{
...
*/
void QXTreeProxyModel::clear_mapping()
{
...
/*
IndexMap::const_iterator QXTreeProxyModelPrivate::create_mapping(
    const QModelIndex &source_parent) const
{
    Q_Q(const QXTreeProxyModel);
...
QModelIndex QXTreeProxyModel::proxy_to_source(const QModelIndex &proxy_index) const{
   if (!proxy_index.isValid()) return QModelIndex(); // for now; we may want to be able to set a root index later
...
QModelIndex QXTreeProxyModel::source_to_proxy(const QModelIndex &source_index) const{
   if (!source_index.isValid()) return QModelIndex(); // for now; we may want to be able to set a root index later
...
/*
bool QXTreeProxyModelPrivate::can_create_mapping(const QModelIndex &source_parent) const{
    if (source_parent.isValid()) {
        QModelIndex source_grand_parent = source_parent.parent();
...
* /
void QXTreeProxyModelPrivate::sort()
{
    Q_Q(QXTreeProxyModel);
...
* /
bool QXTreeProxyModelPrivate::update_source_sort_column()
{
    Q_Q(QXTreeProxyModel);
...
* /
void QXTreeProxyModelPrivate::sort_source_rows(
    QVector<int> &source_rows, const QModelIndex &source_parent) const
{
    Q_Q(const QXTreeProxyModel);
    if (source_sort_column >= 0) {
        if (sort_order == Qt::AscendingOrder) {
            QXTreeProxyModelLessThan lt(source_sort_column, source_parent, model, q);
            qStableSort(source_rows.begin(), source_rows.end(), lt);
        } else {
            QXTreeProxyModelGreaterThan gt(source_sort_column, source_parent, model, q);
...
* /
QVector<QPair<int, int > > QXTreeProxyModelPrivate::proxy_intervals_for_source_items(
    const QVector<int> &source_to_proxy, const QVector<int> &source_items) const
{
...
* /
void QXTreeProxyModelPrivate::remove_source_items(
    QVector<int> &source_to_proxy, QVector<int> &proxy_to_source,
    const QVector<int> &source_items, const QModelIndex &source_parent,
...
* /
void QXTreeProxyModelPrivate::remove_proxy_interval(
...
* /
QVector<QPair<int, QVector<int > > > QXTreeProxyModelPrivate::proxy_intervals_for_source_items_to_add(
    const QVector<int> &proxy_to_source, const QVector<int> &source_items,
    const QModelIndex &source_parent, Qt::Orientation orient) const
{
    Q_Q(const QXTreeProxyModel);
...
* /
void QXTreeProxyModelPrivate::insert_source_items(
    QVector<int> &source_to_proxy, QVector<int> &proxy_to_source,
    const QVector<int> &source_items, const QModelIndex &source_parent,
    Qt::Orientation orient, bool emit_signal)
{
...
* /
void QXTreeProxyModelPrivate::source_items_inserted(
    const QModelIndex &source_parent, int start, int end, Qt::Orientation orient)
{
    ...
    if (start < 0 || start > source_to_proxy.size()) {
        qWarning("QXTreeProxyModel: invalid inserted rows reported by source model");
...
* /
void QXTreeProxyModelPrivate::source_items_about_to_be_removed(
    const QModelIndex &source_parent, int start, int end, Qt::Orientation orient)
{
    if ((start < 0) || (end < 0))
        return;
...
* /
void QXTreeProxyModelPrivate::source_items_removed(
    const QModelIndex &source_parent, int start, int end, Qt::Orientation orient)
{
    if ((start < 0) || (end < 0))
        return;
    ...
    if (proxy_count > source_to_proxy.size()) {
        ...
        qWarning("QXTreeProxyModel: inconsistent changes reported by source model");
        remove_from_mapping(source_parent);
        Q_Q(QXTreeProxyModel);
        q->reset();
        return;
...
* /
void QXTreeProxyModelPrivate::updateChildrenMapping(const QModelIndex &source_parent, Mapping *parent_mapping,
                                                         Qt::Orientation orient, int start, int end, int delta_item_count, bool remove)
{
...
/*!
  \internal
* /
void QXTreeProxyModelPrivate::proxy_item_range(
    const QVector<int> &source_to_proxy, const QVector<int> &source_items,
    int &proxy_low, int &proxy_high) const
{
    proxy_low = INT_MAX;
...
/*!
  \internal
* /
void QXTreeProxyModelPrivate::build_source_to_proxy_mapping(
    const QVector<int> &proxy_to_source, QVector<int> &source_to_proxy) const
{
...
*/
QModelIndexPairList QXTreeProxyModel::store_persistent_indexes()
{
...
*/
void QXTreeProxyModel::update_persistent_indexes(const QModelIndexPairList &source_indexes)
{
...
/*!
  ...
  new filter.
* /
void QXTreeProxyModelPrivate::filter_changed(const QModelIndex &source_parent)
{
    IndexMap::const_iterator it = source_index_mapping.constFind(source_parent);
    if (it == source_index_mapping.constEnd())
        return;
...
* /
QSet<int> QXTreeProxyModelPrivate::handle_filter_changed(
    QVector<int> &source_to_proxy, QVector<int> &proxy_to_source,
    const QModelIndex &source_parent, Qt::Orientation orient)
{
    Q_Q(QXTreeProxyModel);
...
void QXTreeProxyModelPrivate::_q_sourceDataChanged(const QModelIndex &source_top_left,
                     const QModelIndex &source_bottom_right)
{
    Q_Q(QXTreeProxyModel);
...
void QXTreeProxyModelPrivate::_q_sourceHeaderDataChanged(Qt::Orientation orientation,
                                                           int start, int end)
{
    Q_Q(QXTreeProxyModel);
...
void QXTreeProxyModelPrivate::_q_sourceAboutToBeReset()
{
    Q_Q(QXTreeProxyModel);
    q->beginResetModel();
}

void QXTreeProxyModelPrivate::_q_sourceReset()
{
    Q_Q(QXTreeProxyModel);
...
void QXTreeProxyModelPrivate::_q_sourceLayoutAboutToBeChanged()
{
    Q_Q(QXTreeProxyModel);
...
void QXTreeProxyModelPrivate::_q_sourceLayoutChanged()
{
    Q_Q(QXTreeProxyModel);
...
void QXTreeProxyModelPrivate::_q_sourceRowsAboutToBeInserted(
...
void QXTreeProxyModelPrivate::_q_sourceRowsInserted(
    const QModelIndex &source_parent, int start, int end)
{
...
void QXTreeProxyModelPrivate::_q_sourceRowsAboutToBeRemoved(
    const QModelIndex &source_parent, int start, int end)
{
...
void QXTreeProxyModelPrivate::_q_sourceRowsRemoved(
    const QModelIndex &source_parent, int start, int end)
{
...
void QXTreeProxyModelPrivate::_q_sourceColumnsAboutToBeInserted(
    const QModelIndex &source_parent, int start, int end)
{
    Q_UNUSED(start);
    Q_UNUSED(end);
...
void QXTreeProxyModelPrivate::_q_sourceColumnsInserted(
    const QModelIndex &source_parent, int start, int end)
{
    Q_Q(const QXTreeProxyModel);
...
void QXTreeProxyModelPrivate::_q_sourceColumnsAboutToBeRemoved(
    const QModelIndex &source_parent, int start, int end)
{
    source_items_about_to_be_removed(source_parent, start, end,
                                     Qt::Horizontal);
...
void QXTreeProxyModelPrivate::_q_sourceColumnsRemoved(
    const QModelIndex &source_parent, int start, int end)
{
    Q_Q(const QXTreeProxyModel);
...
/*!
    ...
    \brief The QXTreeProxyModel class provides support for sorting and
    filtering data passed between another model and a view.

    \ingroup model-view

    QXTreeProxyModel can be used for sorting items, filtering out items,
    ...
    a QXTreeProxyModel, call setSourceModel() with the \c MyItemModel as
    argument, and install the QXTreeProxyModel on the view:
    ...
    QXTreeProxyModel are applied to the original model.

    The QXTreeProxyModel acts as a wrapper for the original model. If you
    ...
    setting the \l{QXTreeProxyModel::dynamicSortFilter}{dynamicSortFilter}
    property.
    ...
    illustrate how to use QXTreeProxyModel to perform basic sorting and
    ...
    either implement sort() in your model, or use a QXTreeProxyModel to
    wrap your model -- QXTreeProxyModel provides a generic sort()
    ...
    default; this can be changed by setting the \l{QXTreeProxyModel::}
    {sortCaseSensitivity} property.

    Custom sorting behavior is achieved by subclassing
    QXTreeProxyModel and reimplementing lessThan(), which is
    ...
    QXTreeProxyModel (or on the original model if it implements sort()).
    For example:

    \snippet doc/src/snippets/qsortfilterproxymodel-details/main.cpp 4

    QXTreeProxyModel can be sorted by column -1, in which case it returns
    to the sort order of the underlying source model.

    \section1 Filtering

    In addition to sorting, QXTreeProxyModel can be used to hide items
    ...
    the \l{QXTreeProxyModel::filterKeyColumn}{filterKeyColumn} property
...
QXTreeProxyModel::QXTreeProxyModel(QObject *parent) : QAbstractProxyModel(parent){
}
...
void QXTreeProxyModel::setSourceModel(QAbstractItemModel *sourceModel){
   ...
   QAbstractItemModel* model(sourceModel);
    ...
    disconnect(model, SIGNAL(modelAboutToBeReset()), this, SLOT(_q_sourceAboutToBeReset()));
    disconnect(model, SIGNAL(modelReset()), this, SLOT(_q_sourceReset()));
    ...
    connect(model, SIGNAL(modelAboutToBeReset()), this, SLOT(_q_sourceAboutToBeReset()));
    connect(model, SIGNAL(modelReset()), this, SLOT(_q_sourceReset()));
...
/*   QModelIndex source_parent = mapToSource(parent); // parent is already mapped at this point
   ...
   if (it.value()->source_rows.count() <= row || it.value()->source_columns.count() <= column) return QModelIndex();
...
{
/*    if (!indexValid(child)) return QModelIndex();
...
int QXTreeProxyModel::rowCount(const QModelIndex &parent) const
{
   ...
   if (parent.isValid() && !source_parent.isValid()) return 0;
...
bool QXTreeProxyModel::hasChildren(const QModelIndex &parent) const{
   QModelIndex source_parent = mapToSource(parent);
   if (parent.isValid() && !source_parent.isValid()) return false;
   if (sourceModel()->hasChildren(source_parent)) return false;

   if (sourceModel()->canFetchMore(source_parent)) return true; //we assume we might have children that can be fetched

/*   QXTreeProxyModelPrivate::Mapping *m = d->create_mapping(source_parent).value();
...
/*!
  \reimp
*/
QVariant QXTreeProxyModel::data(const QModelIndex &index, int role) const{
   QModelIndex source_index = mapToSource(index);
   if (index.isValid() && !source_index.isValid()) return QVariant();
   return sourceModel()->data(source_index, role);}

/*!
  \reimp
*/
bool QXTreeProxyModel::setData(const QModelIndex &index, const QVariant &value, int role){
   QModelIndex source_index = mapToSource(index);
   if (index.isValid() && !source_index.isValid()) return false;
   return sourceModel()->setData(source_index, value, role);}

/*!
  \reimp
*/
QVariant QXTreeProxyModel::headerData(int section, Qt::Orientation orientation, int role) const{
...
/*!
  \reimp
*/
bool QXTreeProxyModel::setHeaderData(int section, Qt::Orientation orientation, const QVariant &value, int role){
/*    IndexMap::const_iterator it = d->create_mapping(QModelIndex());
    if (it.value()->source_rows.count() * it.value()->source_columns.count() > 0)
...
/*!
  \reimp
*/
bool QXTreeProxyModel::dropMimeData(const QMimeData *data, Qt::DropAction action, int row, int column, const QModelIndex &parent)
{
    if ((row == -1) && (column == -1))
...
/*!
  \reimp
*/
void QXTreeProxyModel::fetchMore(const QModelIndex &parent){
   QModelIndex source_parent;
   if (parent.isValid()) source_parent = mapToSource(parent);
   sourceModel()->fetchMore(source_parent);}

/*!
  \reimp
*/
bool QXTreeProxyModel::canFetchMore(const QModelIndex &parent) const{
   QModelIndex source_parent;
   if (parent.isValid()) source_parent = mapToSource(parent);
...
Qt::ItemFlags QXTreeProxyModel::flags(const QModelIndex &index) const{
   ...
   if (index.isValid()) source_index = mapToSource(index);
   return sourceModel()->flags(source_index);}

/*!
  \reimp
*/
QModelIndex QXTreeProxyModel::buddy(const QModelIndex &index) const{
   if (!index.isValid()) return QModelIndex();
   QModelIndex source_index = mapToSource(index);
   QModelIndex source_buddy = sourceModel()->buddy(source_index);
   if (source_index == source_buddy) return index;
   return mapFromSource(source_buddy);}

/*!
  \reimp
*/
QModelIndexList QXTreeProxyModel::match(const QModelIndex &start, int role, const QVariant &value, int hits, Qt::MatchFlags flags) const {
   return QAbstractProxyModel::match(start, role, value, hits, flags);}

/*!
  \reimp
*/
QSize QXTreeProxyModel::span(const QModelIndex &index) const{
   QModelIndex source_index = mapToSource(index);
   if (index.isValid() && !source_index.isValid()) return QSize();
...
/*!
   \since 4.3
    ...
    \sa invalidateFilter()
*/
void QXTreeProxyModel::invalidate()
{
...
{
    return proxy_to_source(proxyIndex);
}

/*!
    Returns the model index in the QXTreeProxyModel given the \a
...
{
    return source_to_proxy(sourceIndex);
}

/*!
  \reimp
*/
QItemSelection QXTreeProxyModel::mapSelectionToSource(const QItemSelection &proxySelection) const{
   return QAbstractProxyModel::mapSelectionToSource(proxySelection);}

/*!
  \reimp
*/
QItemSelection QXTreeProxyModel::mapSelectionFromSource(const QItemSelection &sourceSelection) const{
